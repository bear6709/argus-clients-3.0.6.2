...
/*
 * $Id: //depot/argus-3.0.6/clients/common/argus_label.c#4 $
...
ArgusLabelRecord (struct ArgusParserStruct *parser, struct ArgusRecordStruct *argus)
{
   ...
   if (found) {
      if (lstruct == NULL) {
         int len = 4 * ((strlen(label) + 3)/4);

         if ((lstruct = ArgusCalloc(1, sizeof(*lstruct))) == NULL)
            ArgusLog (LOG_ERR, "RaProcessRecord: ArgusCalloc error %s", strerror(errno));
         ...
         argus->dsrs[ARGUS_LABEL_INDEX] = (struct ArgusDSRHeader*) lstruct;
...
RaInsertAddress (struct ArgusParserStruct *parser, struct ArgusLabelerStruct *labeler, struct RaAddressStruct *tree, struct RaAddressStruct *node, int status)
{
   ...
   if ((labeler != NULL) && (node != NULL)) {
      if (tree == NULL)
         tree = ArgusAddrTree[node->addr.type];

      node->status |= ARGUS_NODE;

      if (tree == NULL) {
         int x = 0, masklen = node->addr.masklen;

         switch (node->addr.type) {
            case AF_INET: {
               if (masklen) {
                  node->addr.mask[x] = 0xFFFFFFFF << (32 - masklen);
               } else
                  node->addr.mask[x] = 0;
               break;
            }
            ...
            switch (tree->addr.type) {
               case AF_INET: {
                  ...
                  if (naddr == taddr) {     // node and tree address are same, but may not be at right part in tree
                     tree->status |= status;
                     if (node->addr.masklen == tree->addr.masklen)  // if so, we have a match, so don't insert
                        return (NULL);
                     ...
                     if (node->addr.masklen > tree->addr.masklen) {  // if node mask is longer, then we'll insert below, which side?
                        ...
                        if (naddr & 0x01) {
                           if (tree->l == NULL) {
                              ...
                              retn = RaInsertAddress (parser, labeler, tree->l, node, status);
                           ...
                           if (tree->r == NULL) {
                              ...
                              retn = RaInsertAddress (parser, labeler, tree->r, node, status);
                        }
...
RaCountryCodeLabel (struct ArgusParserStruct *parser, struct ArgusRecordStruct *argus)
{
   ...
   if (argus != NULL) {
      ...
      if (cocode == NULL)
         if ((cocode = ArgusMalloc(sizeof(*cocode))) == NULL)
            ArgusLog (LOG_ERR, "RaProcessRecord: ArgusCalloc error %s", strerror(errno));
