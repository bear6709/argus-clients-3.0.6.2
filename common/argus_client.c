...
/* 
 * $Id: //depot/argus-3.0.6/clients/common/argus_client.c#4 $
 * $DateTime: 2012/06/15 10:05:41 $
 * $Change: 2423 $
 */
         ...
         while (input->ArgusSaslBufCnt) {
            ...
            if (bytes > 0) {
               ...
               while (!retn && !done && !parser->RaParseDone) {
                  ...
                  if (rec) {
                     int len;
                     if ((len = ArgusHandleDatum (ArgusParser, input, rec, &ArgusParser->ArgusFilterCode)) == -1) {
                        retn = 1;
                     } else {
                        input->offset += len;
   ...
   if (cnt > 0) {
      ...
      while (!done) {
         ...
         if (rec && !done && !parser->RaParseDone) {
            int len = 0;
            if ((len = ArgusHandleDatum (ArgusParser, input, rec, &ArgusParser->ArgusFilterCode)) < 0) {
               retn = 1;
               done = 1;
            } else {
...
#ifdef ARGUSDEBUG
   ArgusDebug (3, "ArgusReadStream(%p) returning", parser);
#endif
}
                           ...
                           case ARGUS_FLOW_CLASSIC5TUPLE: {
                              ...
                              switch (flow->hdr.argus_dsrvl8.qual & 0x1F) {
                                 ...
                                 case ARGUS_TYPE_WLAN:
                                    dlen = sizeof(canon->flow.wlan_flow);
                                 ...
                                 case ARGUS_TYPE_ISIS:
                                    dlen = sizeof(canon->flow.isis_flow);
...
#ifdef ARGUSDEBUG
   ArgusDebug (5, "RaNewBin(%p, %p) returns %p\n", rbps, ns, retn);
#endif
   return (retn);
}
...
ArgusHistoTallyMetric (struct ArgusParserStruct *parser, struct ArgusRecordStruct *ns, double value)
{
   ...
   if (parser && (ns != NULL)) {
      ...
      if (value < start) {
         ...
         if (value < (parser->RaHistoEnd + bsize))
            i++;
      }
   }
...
unsigned int
ArgusMergeAddress(unsigned int *a1, unsigned int *a2, int type, int dir, char *masklen)
{
   unsigned int retn = 0;
...
ArgusMergeLabel(struct ArgusLabelStruct *l1, struct ArgusLabelStruct *l2, char *buf, int len, int type)
{
   ...
   struct ArgusLabelObject l1labs[256], l2labs[256];
   ...
   bcopy(l1->l_un.label, l1buf, l1len); l1buf[l1len] = '\0';
   bcopy(l2->l_un.label, l2buf, l2len); l2buf[l2len] = '\0';

// first parse all the attributes and values. This system limits the 
// number of attributes to 256 and 4 values per attribute.
   ...
   for (i = 0; i < l1labsindex; i++) {
      if (l1labs[i].object != NULL) {
         ...
         if (l1labs[i].values[0] != NULL) {
            ...
            while ((y < 4) && (l1labs[i].values[y] != NULL)) {
   ...
   for (i = 0; i < l2labsindex; i++) {
      if (l2labs[i].object != NULL) {
         ...
         if (l2labs[i].values[0] != NULL) {
            ...
            while ((y < 4) && (l2labs[i].values[y] != NULL)) {
               ...
               if ((n1 != NULL) && (n2 != NULL)) {
                  ...
                  if ((n1 != NULL) && (n2 != NULL)) {
                     switch (n1->hdr.subtype) {
                        case ARGUS_TCP_INIT: {
                           struct ArgusTCPInitStatus *t1 = (struct ArgusTCPInitStatus *)&n1->net_union.tcp;
                           struct ArgusTCPInitStatus tcpstatusbuf, *tcps = &tcpstatusbuf;
                           ...
                           switch (n2->hdr.subtype) {
                              ...
                              case ARGUS_TCP_STATUS: {
                                 ...
                                 tobj->options      = tcps->options;
                                 tobj->src.status   = tcps->status;
                                 tobj->src.seqbase  = tcps->seqbase;
                                 tobj->src.win      = tcps->win;
                                 tobj->src.flags    = tcps->flags | t2->src;
                                 tobj->src.winshift = tcps->winshift;
                                 tobj->dst.flags    = t2->dst;
                                 break;
                              }
                              case ARGUS_TCP_PERF: {
...
// first does this direction need adjustment, if xstime > esecs then we need to pass.
                  ...
                  if (nadp->sploss > 0)
                     ArgusAdjustSrcLoss(ns, nadp->sploss);
                  if (nadp->dploss > 0)
                     ArgusAdjustDstLoss(ns, nadp->dploss);
...
ArgusCopyAggregator (struct ArgusAggregatorStruct *agg) 
{
   ...
   while (agg != NULL) {
      if ((tagg = (struct ArgusAggregatorStruct *) ArgusCalloc (1, sizeof(*tagg))) == NULL)
      ...
      if (agg->hstruct.buf != NULL) {
         if ((tagg->hstruct.buf = (unsigned int *) ArgusCalloc (1, agg->hstruct.len)) == NULL)
            ArgusLog (LOG_ERR, "ArgusCopyAggregator: ArgusCalloc error %s", strerror(errno));

         bcopy (agg->hstruct.buf, tagg->hstruct.buf, agg->hstruct.len);
      }
      ...
      if (agg->filterstr != NULL) tagg->filterstr = strdup(agg->filterstr);
...
ArgusFetchSrcMac (struct ArgusRecordStruct *ns)
{
   ...
   if (m != NULL) {
      ((u_char *)&value)[0] = m->mac.mac_union.ether.ehdr.ether_shost[0];
      ((u_char *)&value)[1] = m->mac.mac_union.ether.ehdr.ether_shost[1];
      ((u_char *)&value)[2] = m->mac.mac_union.ether.ehdr.ether_shost[2];
      ((u_char *)&value)[3] = m->mac.mac_union.ether.ehdr.ether_shost[3];
      ((u_char *)&value)[4] = m->mac.mac_union.ether.ehdr.ether_shost[4];
      ((u_char *)&value)[5] = m->mac.mac_union.ether.ehdr.ether_shost[5];
...
ArgusFetchDstMac (struct ArgusRecordStruct *ns)
{
   ...
   if (m != NULL) {
      ((u_char *)&value)[0] = m->mac.mac_union.ether.ehdr.ether_dhost[0];
      ((u_char *)&value)[1] = m->mac.mac_union.ether.ehdr.ether_dhost[1];
      ((u_char *)&value)[2] = m->mac.mac_union.ether.ehdr.ether_dhost[2];
      ((u_char *)&value)[3] = m->mac.mac_union.ether.ehdr.ether_dhost[3];
      ((u_char *)&value)[4] = m->mac.mac_union.ether.ehdr.ether_dhost[4];
      ((u_char *)&value)[5] = m->mac.mac_union.ether.ehdr.ether_dhost[5];
...
ArgusNewSorter (struct ArgusParserStruct *parser)
{
   ...
   if ((parser == NULL) || (parser->RaSortOptionIndex == 0)) 
