...
/* 
 * $Id: //depot/argus-3.0.6/clients/common/argus_import.c#3 $
...
struct ArgusRecord *ArgusParseCiscoRecordV7 (struct ArgusInput *, u_char **);
struct ArgusRecord *ArgusParseCiscoRecordV8 (struct ArgusInput *, u_char **);
struct ArgusRecord *ArgusParseCiscoRecordV9 (struct ArgusInput *, u_char **);

struct ArgusRecord *ArgusParseCiscoRecordV9Data (struct ArgusInput *, u_char **);
...
struct ArgusRecord * 
ArgusParseCiscoRecordV7 (struct ArgusInput *input, u_char **ptr)
{
   struct ArgusRecord *argus = ArgusNetFlowArgusRecord;
...
struct ArgusRecord *
ArgusParseCiscoRecordV8 (struct ArgusInput *input, u_char **ptr)
{
...
struct ArgusRecord *
ArgusParseCiscoRecordV9Data (struct ArgusInput *input, u_char **ptr)
{
   struct ArgusRecord *retn = NULL;
...
struct ArgusRecord *
ArgusParseCiscoRecordV9 (struct ArgusInput *input, u_char **ptr)
{
   struct ArgusRecord *retn = NULL;
// CiscoFlowHeaderV9_t *hdrPtrV9 = (CiscoFlowHeaderV9_t *) ArgusNetFlowRecordHeader;
   int done = 0, offset = 0, flowset_id, flowset_len;

   *ptr += sizeof(CiscoFlowEntryV9_t);

   while (!done) {
      CiscoFlowEntryV9_t  *entryPtrV9 = (CiscoFlowEntryV9_t *) (*ptr + offset);
      ...
      switch (flowset_id) {
         ...
         default: {
            if (flowset_id >= k_CiscoV9MinRecordFlowsetId) {
               if ((retn = ArgusParseCiscoRecordV9Data(input, ptr)) == NULL) {
               }
            }
            done++;
            break;
            ...
            default: {
               ...
               switch (input->ArgusReadCiscoVersion) {
                  ...
                  case CISCO_VERSION_9: {
                     struct ArgusRecord *argus = input->ArgusCiscoNetFlowParse (input, &input->ArgusReadPtr);
                     if  (argus != NULL)
                        if (ArgusHandleDatum (ArgusParser, input, argus, &ArgusParser->ArgusFilterCode) < 0)
                           return(1);
                     break;
                  }
               }
   ...
   if ((cnt = recvfrom (input->fd, input->ArgusReadPtr, input->ArgusReadSocketSize, 0L, &from, &fromlen)) > 0) {
      ...
      switch (input->ArgusReadCiscoVersion = ntohs(*sptr)) {
         ...
         case CISCO_VERSION_7: {
            ...
            CiscoFlowHeaderV7_t *nfptr = (CiscoFlowHeaderV7_t *) sptr;
         ...
         case CISCO_VERSION_9: {
            ...
            CiscoFlowHeaderV9_t *nfptr = (CiscoFlowHeaderV9_t *) sptr;
            ...
            ArgusNetFlow->sysUptime        = ntohl(nfptr->sysUptime);
            ArgusNetFlow->unix_secs        = ntohl(nfptr->unix_secs);
            ArgusNetFlow->package_sequence = ntohl(nfptr->package_sequence);
            ArgusNetFlow->source_id        = ntohl(nfptr->source_id);
            ArgusNetFlowRecordHeader = ptr;
            ptr = (unsigned char *) (nfptr + 1);
            count = ArgusNetFlow->count;
            break;
