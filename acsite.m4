dnl
dnl Copyright (C) 2000-2012 QoSient, LLC.
dnl
...
dnl
dnl $Id: //depot/argus-3.0.6/clients/acsite.m4#2 $
dnl $DateTime: 2012/05/09 10:17:30 $
dnl $Change: 2405 $
dnl
     ...
     if test "$ac_cv_found_el_init" = yes; then
        ...
        if test "$ac_cv_found_readline" != no; then
           AC_CHECK_LIB(readline, rl_replace_line,
                        ac_cv_found_rl_replace_line=yes,
                        ac_cv_found_rl_replace_line=no)

           $1="-lreadline"
           AC_DEFINE([ARGUS_READLINE], [], [Using System Readline Library])
           if test "$ac_cv_found_rl_replace_line" = yes; then
              AC_DEFINE([ARGUS_READLINE_REPLACE_LINE], [], [Using System Readline Replace Line routine])
           fi
        fi
     fi
     LIBS="$LBL_LIBS"
...
# Rob Siemborski
# $Id: //depot/argus-3.0.6/clients/acsite.m4#2 $
