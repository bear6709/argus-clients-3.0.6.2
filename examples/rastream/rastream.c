 ...
 *
 * 
 * $Id: //depot/argus-3.0.6/clients/examples/rastream/rastream.c#2 $
 * $DateTime: 2012/05/07 12:29:30 $
 * $Change: 2399 $
 */
...
#ifdef ARGUSDEBUG
                  ArgusDebug (2, "ArgusClientTimeout() RaBinProcess: Bflag %f rtime %lld start %d.%06d size %.06f arraylen %d count %d index %d\n",
                     ArgusParser->Bflag, rtime, btime->src.start.tv_sec, bin->stime.tv_usec, rbps->size/1000000.0, rbps->arraylen, 0, rbps->index);
#endif
...
usage ()
{
   ...
   fprintf (stdout, "Rastream Version %s\n", version);
   fprintf (stdout, "usage: %s \n", ArgusParser->ArgusProgramName);
   fprintf (stdout, "usage: %s [options] -S remoteServer  -f rastream.sh [- filter-expression]\n", ArgusParser->ArgusProgramName);
   fprintf (stdout, "usage: %s [options] -r argusDataFile -f rastream.sh [- filter-expression]\n\n", ArgusParser->ArgusProgramName);

   fprintf (stdout, "options: -B <sec>           Delay time before closing files and processing.\n");
#if defined (ARGUSDEBUG)
   fprintf (stdout, "         -D <level>         specify debug level\n");
#endif
   fprintf (stdout, "         -f <script>        run <script> against output file after closing.\n");
   fprintf (stdout, "         -h                 print help.\n");

   fprintf (stdout, "         -M <mode>          supported modes of operation:\n");
   fprintf (stdout, "            time n[smhdwmy]\n");
   ...
   fprintf (stdout, "         -w <extended.file> write output to rasplit.1 style output archive.\n");
   fflush (stdout);
...
#ifdef ARGUSDEBUG
   ArgusDebug (3, "RaSendArgusRecord (%p) done\n", argus); 
#endif
   return (retn);
}
