...
/* 
 * $Id: //depot/argus-3.0.6/clients/examples/ramysql/rasql.c#2 $
 * $DateTime: 2012/04/18 14:18:40 $
 * $Change: 2370 $
 */
...
RaMySQLInit ()
{
   ...
   bzero(&RaTableExistsNames, sizeof(RaTableExistsNames));
...
*/
   ...
   if ((input = ArgusParser->ArgusInputFileList) != NULL) {
      char *table = input->filename;

// assumption here is that if "-r tableSpec" is used, without the
// mysql: keyword, with rasql.c we should still be able to find
// the string as if it were a table.
...
{
   char ArgusSQLStatement[1024];
   char buf[0x10000], sbuf[0x10000], *table;
   MYSQL_RES *mysqlRes;
   struct timeval now;
   int ArgusTimeSchema = 0;
   int retn, x, i;
   ...
   if (ArgusParser->tflag) {
      ...
      if (ArgusParser->ArgusSQLStatement != NULL) {
         ...
         snprintf (ArgusSQLStatement, 1024, "%s >= %d and %s < %d", timeField, (int)ArgusParser->startime_t.tv_sec, timeField, (int)ArgusParser->lasttime_t.tv_sec);
