...
/* 
 * $Id: //depot/argus-3.0.6/clients/examples/ramysql/rasqltimeindex.c#1 $
...
void RaArgusInputComplete (struct ArgusInput *input) {
   int cnt, retn, fileindex = 0, filestatus = -1;
   ...
   if ((input != NULL) && ((ArgusProbes) && (ArgusProbes->queue) && (ArgusProbes->queue->count > 0))) {
         ...
         do {
            ...
            if ((retn = mysql_real_query(&mysql, buf, strlen(buf))) != 0) 
               ...
               if (!found) {
                  ...
                  if ((retn = mysql_real_query(&mysql, buf, strlen(buf))) != 0)
                     ArgusLog(LOG_ERR, "mysql_real_query error %s: %s", buf, mysql_error(&mysql));
               }
            }
         } while (!found && !error++);
     ...
     if ((pathptr = realpath(input->filename, filename)) != NULL) {
      ...
      if (filestatus == 2) {
         if (ArgusParser->vflag)
            ArgusLog(LOG_INFO, "file %s already processed", ArgusInput->filename);
      ...
      if ((retn = mysql_real_query(&mysql, buf, strlen(buf))) != 0)
         ...
         if ((mysqlRes = mysql_store_result(&mysql)) != NULL) {
            ...
            ArgusLog(LOG_ERR, "mysql_store_result error %s", mysql_error(&mysql));
      }
      ...
      if (parser->ArgusInputFileList != NULL) {
         ...
         do {
            ...
            if (!((filestatus == 1) || (filestatus == 2))) {
               ...
               ArgusLog(LOG_INFO, "file %s already processed", input->filename);
