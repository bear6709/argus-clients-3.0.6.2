 ...
 * QoSient, LLC
 *
 * $Id: //depot/argus-3.0.6/clients/examples/rafilter/rafilteraddr.c#1 $
...
usage ()
{
   ...
   fprintf (stdout, "usage: %s -f address.file [raoptions] -S remoteServer  [- filter-expression]\n", ArgusParser->ArgusProgramName);
   fprintf (stdout, "usage: %s -f address.file [raoptions] -r argusDataFile [- filter-expression]\n", ArgusParser->ArgusProgramName);
   fflush (stdout);
   exit(1);
}
